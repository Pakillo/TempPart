...
### Break up script: prepare_data.R ###

# Import dataset
raw_data <- read.csv(here::here("data", "JPHO_phototraping_definitive.csv"),
                     skip = 18, header = TRUE, sep = ";", as.is = TRUE)
...
levels(raw_data$binomial) ### Check this formally (assertr) ###

# New variable with hms
raw_data$hms <- as_hms(raw_data$time)

### This generates warning at row 2534:
raw_data[2534, c("time", "hms")]
raw_data$time[2534] <- "02:21:05"
raw_data$hms <- as_hms(raw_data$time)
###
...
levels(raw_data$area) ### Check formally (assertr)
...
library("dplyr")
raw_data <- raw_data %>%
  mutate(across(c(area, ind, sp, behavior), as.factor))
###
...
### Check data
library("assertr")
meso_data_check <- meso_data %>%
  assert(in_set(c("SABCOL", "SABMAR", "SABOJI")), area) %>%
  verify(between(meso_data$date, as.Date("2018-01-01"), as.Date("2020-12-31"))) %>%
  assert(in_set(c("Alectoris rufa", "Anthus pratensis")), binomial)
# note there are typos in species names
# etc
###
...
### Break up script: species_detection.R ###
...
### dplyr alternative:
mesocarniv.sp <- c("Genetta genetta", "Vulpes vulpes", "Meles meles", "Herpestes ichneumon")
mesocarnivores <- meso_data %>%
  filter(binomial %in% mesocarniv.sp)
###
...
### dplyr alternative:
preys.sp <- c("Oryctolagus cuniculus", "rodent", "Turdus philomelos",
              "Turdus merula", "Alectoris rufa")
preys <- meso_data %>%
  filter(binomial %in% preys.sp)
preys %>%
  group_by(binomial, area) %>%
  summarise(n())
...
### This gives error because Date_Time must be character or factor
meso_data$Date_Time_chr <- as.character(meso_data$Date_Time)
activityDensity(recordTable = meso_data, allSpecies = TRUE, speciesCol = "binomial", recordDateTimeCol = "Date_Time_chr",
                recordDateTimeFormat = "%Y-%m-%d %H:%M:%S", plotR = TRUE)
###
...
### Here starts some repeated code ###
...
### Remember the DRY principle (Don't Repeat Yourself). Trying to refactor:

# Write function (and save apart):
plot_overlap_2spp <- function(sp1, sp2, dataset = meso_data) {

  # Define colors per species
  color.sp <- data.frame(
    sp = c("Herpestes ichneumon", "Vulpes vulpes", "Genetta genetta", "Meles meles"),
    color = c("#F0E68C", "#4682B4", "#66CDAA", "#9370DB")
  )
  sp1.color = color.sp$color[color.sp$sp == sp1]
  sp2.color = color.sp$color[color.sp$sp == sp2]

  # Get times for each species
  sp1.times <- dataset$solar[dataset$binomial == sp1]
  sp2.times <- dataset$solar[dataset$binomial == sp2]

  # Plot function
  overlapPlot(sp1.times, sp2.times,
              rug = TRUE, linetype = c(1, 1),
              linecol = c(sp1.color, sp2.color),
              ylim = c(0.00, 0.13),
              olapcol = "#F5FFFA",
              linewidth = c(1, 1),
              main = paste(sp1, "vs", sp2, sep = " "))

}


# Run function:
plot_overlap_2spp("Vulpes vulpes", "Meles meles")
plot_overlap_2spp("Genetta genetta", "Vulpes vulpes")
...
### Repeated code below: try refactoring

# Write function
calc_overlap <- function(sp.pair, dataset = meso_data) {

  sp1.timerad <- dataset$TimeRad[dataset$binomial == sp.pair[1]]
  sp2.timerad <- dataset$TimeRad[dataset$binomial == sp.pair[2]]

  # Coding this condition formally below - won't fail (ie. avoid human error)
  if (min(length(sp1.timerad), length(sp2.timerad)) < 50) {
    estimator <- "Dhat1"
  } else {
    estimator <- "Dhat4"
  }

  overlap <- overlapEst(sp1.timerad, sp2.timerad, type = estimator)
  overlap

}


## Species pairs to compare:
sp.pairs <- list(
  GV = c("Genetta genetta", "Vulpes vulpes"),
  GH = c("Genetta genetta", "Herpestes ichneumon"),
  GM = c("Genetta genetta", "Meles meles"),
  VH = c("Vulpes vulpes", "Herpestes ichneumon")
  #etc
)

overlap.pairs <- lapply(sp.pairs, calc_overlap)
overlap.pairs
...
min(length(genetta$TimeRad), length(vulpes$TimeRad)) # Sample size < 50 so we should use Dthat 1 (check because new detections could be added to the dataset)  ### Problematic
